...
- `server/src/source-watcher.ts` — Per-source `SourceWatcher` class for `kind: 'claude-code'` sources. Scans existing JSONL files, watches via chokidar (directory-level watch with a `.jsonl` filter — chokidar v4 dropped glob support), links subagents within the source, tags every parsed `Session` with its `sourceId`. It retains only `SessionMeta` per session (plus the parent's `Agent` tool-call inputs, the one piece of body data subagent linking needs) — bodies live in the archive and are loaded on demand, so the watcher map doesn't pin the whole corpus in the heap. `getAllMeta()` is what the registry reads, and the `session-created`/`session-updated` payload is a `SessionMeta`, not a full `Session`. Every parse (initial scan or a live file event) writes through to `db.archive.put()` with the raw lines and file fingerprint, and indexes non-subagent sessions into FTS; `stop()` flushes any deferred archive body writes. On the initial scan, a file whose fingerprint (size + mtime) still matches the archive's stored one skips a full re-parse and is served straight from the archived summary and body instead. Scanning goes through a shared `Limiter` so a startup sweep across many sources parses a bounded number of transcripts at a time instead of all of them at once. Optional constructor options (`{ watch, transformSession, rescan, limiter }`) let a `StoreSetWatcher`-driven child disable live watching, rewrite each session via `applyOrigin`, (`rescan: true`) force a full re-parse instead of trusting the fingerprint, and share the registry's parse-concurrency cap.
...
- `server/src/opencode-watcher.ts` — `OpenCodeWatcher` class for `kind: 'opencode'` sources. Polls `opencode.db` and its `-wal` file's mtime instead of watching a directory; `pollOnce()` is public for deterministic tests. Like `SourceWatcher` it retains `SessionMeta` only and emits meta payloads, deferring its emits until after `linkSubagents()` so a parent's `subagents` is populated in the payload.
...
Tests are in `server/test/`. Run with `pnpm test` or `cd server && npx vitest run`. Total: 328 tests across 22 files.
...
- `limiter.test.ts` — 5 tests for `Limiter`: runs up to the limit concurrently, never over-subscribes when a caller arrives in the window where a slot is being handed off, frees the slot after a rejected task, preserves FIFO order among waiters, and defaults to `DEFAULT_PARSE_LIMIT`.
